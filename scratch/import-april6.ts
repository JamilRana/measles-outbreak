...
// Changed to April 6 as per user instruction
const reportDate = new Date(Date.UTC(2026, 3, 6))
...
async function main() {
  ...
  for (const r of rawReports) {
    let facility = await prisma.facility.findFirst({
      where: {
        AND: [
            { district: { contains: r.district.trim(), mode: 'insensitive' } },
    ...
    if (!facility) {
      ...
      facility = await prisma.facility.create({
        data: {
          ...
          facilityCode: `CSO_${r.district.toUpperCase().replace(/\s+/g, '_')}`,
          division: r.division,
          district: r.district,
          upazila: r.district,
  ...
  console.log("April 6 Import finished.")
}
