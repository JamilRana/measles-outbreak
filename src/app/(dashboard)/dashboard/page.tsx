  ...
  return (
    <div className="p-8 max-w-7xl mx-auto space-y-8 pb-16">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <h1 className="text-3xl font-extrabold text-slate-900 tracking-tight">{t('dashboard.title')}</h1>
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="flex gap-2">
            <button onClick={handleExportPDF} className="flex items-center gap-2 bg-[#f43f5e] hover:bg-[#e11d48] text-white px-5 py-2.5 rounded-xl font-bold shadow-lg shadow-rose-500/20"><Download className="w-4 h-4" /> {t('common.pdf')}</button>
            <button onClick={handleExportExcel} className="flex items-center gap-2 bg-[#10b981] hover:bg-[#059669] text-white px-5 py-2.5 rounded-xl font-bold shadow-lg shadow-emerald-500/20"><Download className="w-4 h-4" /> {t('common.excel')}</button>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <StatsCard title={t('stats.suspected')} value={totals.suspected} icon={<Users className="w-6 h-6" />} color="indigo" />
        <StatsCard title={t('stats.confirmed')} value={totals.confirmed} icon={<CheckCircle2 className="w-6 h-6" />} color="emerald" />
        <StatsCard title={t('stats.mortality')} value={totals.deaths} icon={<Skull className="w-6 h-6" />} color="rose" />
        <StatsCard title={t('stats.hospitalized')} value={totals.hospitalized} icon={<Hospital className="w-6 h-6" />} color="amber" />
      </div>
      ...
      <div className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="px-8 py-6 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
          <div className="flex items-center gap-3">
             ...
             <h3 className="text-xl font-bold text-slate-800">{t('charts.detailedSubmissions')}</h3>
          </div>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50/80 text-slate-500 uppercase text-[10px] font-black tracking-[0.1em]">
                <th className="px-8 py-4 w-12 text-center">{t('charts.export')}</th>
                <th className="px-6 py-4">{t('charts.divisionDistrict')}</th>
                ...
                {session?.user.role === "ADMIN" && <th className="px-6 py-4 text-center">{t('charts.admin')}</th>}
              </tr>
            </thead>
          ...
          {!loading && reports.length === 0 && <div className="p-20 text-center text-slate-400 font-medium flex flex-col items-center gap-4"><CloudOff className="w-12 h-12 text-slate-200" /><p>{t('charts.noReportsDate')}</p></div>}
        </div>
      </div>
          ...
          <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="bg-white rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden">
               <div className="px-8 py-6 border-b border-slate-100 flex items-center justify-between">
                  <h2 className="text-xl font-bold text-slate-800">{t('dashboard.editEntry')}: {editingReport.facilityName}</h2>
               ...
               <form onSubmit={handleUpdateReport} className="p-8 space-y-6">
                  <div className="grid grid-cols-2 gap-4">
                     <EditInput label={t('report.fields.suspected24h')} name="suspected24h" value={editingReport.suspected24h} onChange={handleEditChange} />
                     <EditInput label={t('report.fields.confirmed24h')} name="confirmed24h" value={editingReport.confirmed24h} onChange={handleEditChange} />
                     <EditInput label={t('report.fields.suspectedDeath24h')} name="suspectedDeath24h" value={editingReport.suspectedDeath24h} onChange={handleEditChange} />
                     <EditInput label={t('report.fields.confirmedDeath24h')} name="confirmedDeath24h" value={editingReport.confirmedDeath24h} onChange={handleEditChange} />
                     <EditInput label={t('report.fields.admitted24h')} name="admitted24h" value={editingReport.admitted24h} onChange={handleEditChange} />
                     <EditInput label={t('report.fields.discharged24h')} name="discharged24h" value={editingReport.discharged24h} onChange={handleEditChange} />
                  </div>
                  <button type="submit" className="w-full bg-indigo-600 hover:bg-indigo-500 text-white font-bold py-4 rounded-xl flex items-center justify-center gap-2 transition-all"><Check className="w-5 h-5" /> {t('dashboard.saveChanges')}</button>
               </form>
            </motion.div>
          </div>
        )}
